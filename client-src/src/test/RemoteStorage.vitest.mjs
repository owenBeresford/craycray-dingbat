...
describe("I can create RemoteStorage", () => {
  ...
  it("I can create it", () => {
    ...
    assertType< DistantStorable>(RemoteStorage, "this is an object of correct type ");
    ...
    rr.loadState()
      .then((dat) => {
        ...
        for (let i = 0; i < dat.length; i++) {
          assert.equal(dat[i].name && dat[i].name.length > 2, " item at " + i + " has a name");
        ...
        assert.isTrue(new Date() - d1 < 1100, "to download data from the local server must take less than 1 s");
        good();
      })
      .catch((e) => {
