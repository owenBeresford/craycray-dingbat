...
export const ExpectedRendering: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(await canvas.findByText("Return to a valid URL.")).toBeVisible();
    // IOIO get relevant aria attribs...
    expect(await canvas.findByText("/spam-for-me")).toBeVisible();
    expect(await canvas.findByText("/spam-for-me")).toHaveClass("bigger");
  },
};
