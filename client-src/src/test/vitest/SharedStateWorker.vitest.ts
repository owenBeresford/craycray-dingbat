...
console.log("First test, pushWhenAble #4  count of items is wrong");
   ...
   await expect( async () => { return await txt.pushWhenAble(dat)}).rejects.toThrowError(/Server sent an error http status 500/);
  });
  ...
  it("Can use SharedStateWorker (ERRR ON URL)", async () => {
    ...
    dat = [];
    await expect( async () => { return await txt.pushWhenAble(dat)}).rejects.toThrowError( );
    } catch(e:unknown ) { console.log("Err? ", (e as Error).message);}
  });
