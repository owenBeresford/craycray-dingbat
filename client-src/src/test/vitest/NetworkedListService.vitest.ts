...
function directConnection(loc:Location|TestLocation ):NetworkedListService {
  ...
  return new NetworkedListService(d4, d3, PASSBACK, [
...
describe("I can use NetworkedListService", () => {
  ...
  it("I can saveAllLists", async () => {
      ...
      await expect( async () => { return await CONN.saveAllLists(); }).rejects.toThrowError("Data is invalid (no details recorded yet).");
