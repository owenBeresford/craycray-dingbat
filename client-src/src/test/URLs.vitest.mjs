...
describe("I can run URLs", () => {
  it("I can create it", () => {
    ...
    assert.isTrue(
      typeof mapURL === "function",
      `this is an object of correct type ${typeof mapURL}`
    );
  });
  ...
  it("I can map with it", () => {
    ...
    assert.isTrue(
      mapURL("allList", OTHER_ID) === "/list-all",
      "[2] this is NOT valid"
    );
    ...
    assert.isTrue(
      mapURL("aList", OTHER_ID) !== "/list/1",
      "[5] this is valid due to type checking"
    );
