...
let SPort = 3001;
let SIpAddr = '192.168.0.35';
let SSLkey = '';
let SSLcert = '';
let serverOptions = {} as { httpsOptions: ShoppingSSLOpts };
if (process.env && process.env.SHOPPING_PORT) {
  SPort = parseInt(process.env.SHOPPING_PORT, 10);
}
if (process.env && process.env.SHOPPING_IPADDR) {
  SIpAddr = process.env.SHOPPING_IPADDR;
}
if (process.env && process.env.SHOPPING_KEY) {
  SSLkey = process.env.SHOPPING_KEY;
}
if (process.env && process.env.SHOPPING_CERT) {
